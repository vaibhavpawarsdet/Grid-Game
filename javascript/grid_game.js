...
var gridGame = {
    ...
    tick: function () {
        ...
        var elapsedTime = parseInt((now - gridGame.startTime) / 1000, 10);
        helper.doc('timerPanel').textContent = elapsedTime;
        timerFunction = setTimeout(gridGame.tick, 1000);
    },
        ...
        for (var i = 0; i < gridSize * gridSize; i++) {
            ...
            li.ondrop = (event) => {
                ...
                if (origin && dest && p) {
                    ...
                    var now = new Date().getTime();
                    helper.doc('stepCount').textContent = ++gridGame.stepCount;
                    document.querySelector('.timeCount').textContent = (parseInt((now - gridGame.startTime) / 1000, 10));
